...
import ru.onetoone.utils.SessionFactoryUtil;
...
public class StudentDAO {
    ...
    public void create(ru.onetoone.model.Student student) {
        Session session = ru.onetoone.utils.SessionFactoryUtil.getSessionFactory().openSession();
    ...
    public ru.onetoone.model.Student readId(int studentId) {
        Session session = ru.onetoone.utils.SessionFactoryUtil.getSessionFactory().openSession();
        session.beginTransaction();
        ru.onetoone.model.Student readStudent = session.get(ru.onetoone.model.Student.class, studentId);
        session.getTransaction().commit();
        session.close();
        return readStudent;
    }
    ...
    public void updateStudentFirstName(int studentId, String firstName) {
        Session session = ru.onetoone.utils.SessionFactoryUtil.getSessionFactory().openSession();
        session.beginTransaction();
        ru.onetoone.model.Student student = session.get(ru.onetoone.model.Student.class, studentId);
